...
# Train Linear + NN models for PTM/Random, multipliers 1/2/5/10, feats 69/126.
# Saves into models_all/{linear,nn}/<TAG>/<mult>x/feats<feats>/

DATA_DIR="${DATA_DIR:-inputs}"
OUT_ROOT="${OUT_ROOT:-models_all}"
...
mkdir -p "${OUT_ROOT}/linear" "${OUT_ROOT}/nn"
...
echo "Done. Linear+NN models saved under: ${OUT_ROOT}/{linear,nn}"
