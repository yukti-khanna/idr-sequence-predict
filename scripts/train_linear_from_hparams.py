...
def main():
    ...
    ap.add_argument("--models-root", default="models_all/linear")
        ...
        for tag in tags:
            for mult in mults:
                ...
                for feats in feats_list:
                    key = hp.get("linear", {}).get(crit, {}).get(tag, {}).get(f"{mult}x", {}).get(f"feats{feats}")
                    if key is None:
                        raise KeyError(f"Missing hparams for linear/{crit}/{tag}/{mult}x/feats{feats}")

                    best = key["best"]

                    X, y = load_bucket_table(infile, feats, args.drop_cols_dir)

                    # Build models from best params
                    # Logistic
                    lr_hp = best["logreg"]
                    logreg = Pipeline([
                    ...
                    # Partial-fit SGDC (final estimator is still SGDClassifier)
                    ...
                    outdir = Path(args.models_root) / crit / tag / f"{mult}x" / f"feats{feats}"
                    save_model(scaled_sgdc, outdir / f"Scaled_SGDC_{tag}_{feats}_feats.txt_model.sav")
                    save_model(logreg, outdir / f"Logistic_Regression_{tag}_{feats}_feats.txt_model.sav")
                    save_model(partial_fit_sgdc, outdir / f"Partial_Fit_SGDC_{tag}_{feats}_feats.txt_model.sav")

                    print("Saved linear:", outdir)

    print("DONE linear training from cached hparams.")
